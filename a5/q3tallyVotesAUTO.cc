...
TallyVotes::TallyVotes( unsigned int group, Printer & printer ): voteFinish(false), group(group), numWaitingVoters(0), printer(&printer) {}
...
TallyVotes::Tour TallyVotes::vote( unsigned int id, Ballot ballot ) {
    ...
    if ( TallyVotes::numWaitingVoters < TallyVotes::group ) {           // Check if this voter is the last one to form a group
        WAITUNTIL( TallyVotes::voteFinish,
                    TallyVotes::printer->print( id, Voter::States::Block, TallyVotes::numWaitingVoters ),
                    TallyVotes::numWaitingVoters -= 1; TallyVotes::printer->print( id, Voter::States::Unblock, TallyVotes::numWaitingVoters )
                  );                                                    // It is NOT, wait for more voters coming
    } else {
        TallyVotes::printer->print( id, Voter::States::Complete );      // It is, print the complete message
        TallyVotes::voteFinish = true;                                  // Raise the flag as a group is completed
        TallyVotes::numWaitingVoters -= 1;
    } // if
